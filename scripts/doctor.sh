...
if ! command -v docker > /dev/null 2>&1; then
    ...
    if ! docker info > /dev/null 2>&1; then
        ...
        CONTAINERS=("template_db" "template_valkey" "template_jaeger" "template_prometheus" "template_grafana")
...
check_container_port $(get_port_from_env "DB_PORT" "5432") "PostgreSQL" "template_db"
check_container_port $(get_port_from_env "REDIS_PORT" "6379") "Valkey" "template_valkey"
...
if docker ps --format '{{.Names}}' 2>/dev/null | grep -q "template_db"; then
    ...
    if docker exec template_db psql -U postgres -d postgres -c "SELECT 1" > /dev/null 2>&1; then
